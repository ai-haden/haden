...
- `Haden.Tests/HardwareLightSeekingConsoleTests.cs`
  ...
  - Behavior change: extracted adaptive seek-cycle behavior into a new `net9.0` Linux review library, added Linux console tests for it, removed the Windows-only branch-added test file, removed Windows-only projects from the active solution, introduced a new headless `Haden.NxtSDK` Linux project for NXT serial protocol operations, and trimmed docs/prompts to Linux-only workflow guidance.
- `global.json`
