...
namespace Haden.NXTRemote.Simulation.Plotting3D
{
    ...
    public struct Point3D : IEquatable<Point3D>
    {
        ...
        {
            return string.Format("[{0}, {1}, {2}]", ((float)Math.Round(X, decimalPlaces)), ((float)Math.Round(Y, decimalPlaces)), ((float)Math.Round(Z, decimalPlaces)));
        }
        /// <summary>
        ...
        {
            uint hashX = (uint)_x.GetHashCode();
            uint hashY = (uint)_y.GetHashCode();
            uint hashZ = (uint)_z.GetHashCode();
